...
public:
    PositiveNegative(){}
    PositiveNegative(const std::string& PN)
    {
        Parse(PN);
    }
...
private:
    void Parse(const std::string& PN)
    {
        ...
        else if (PN=="Negative"||PN=="NEGATIVE"||PN=="N"||PN=="-") m_positive=false;
        else
        {
...
int main(int argc, char** argv)
{
    ...
    for (Long64_t evt=0; evt < NbrEvents;++evt)
    {
        ...
        for(unsigned int ch=0;ch!=event->Channels.size();++ch)
        {
            ...
            //std::cout<<" Mean : "<<meanstd.first<<" STD : "<<meanstd.second<<std::endl;
            // selected with be updated each time we make some selection... For now it's the same as Waveform one but in Red !!!
            ...
            /*for(std::size_t Iter=0;Iter!=10;++Iter)*/
            {
                ...
                // Part 1
                // Calculate the mean/std for positive part if signal should be POSITIVE or the negative one if signal should be NEGATIVE
                for(std::size_t i=0;i!=event->Channels[ch].Data.size();++i)
                {
                    ...
                    else event->Channels[ch].Data[i]=0;
                }
                ...
                std::pair<std::pair<double,double>,std::pair<double,double>> meanstd=MeanSTD(event->Channels[ch]/*,SignalWindow*/);
                //std::cout<<"Iteration "<<Iter<<" Mean  : "<<meanstd.first<<" STD : "<<meanstd.second<<std::endl;
                // Part 2
                ...
                for(std::size_t i=0;i!=event->Channels[ch].Data.size();++i)
                {
                    ...
                    else event->Channels[ch].Data[i]=event->Channels[ch].Data[i]-meanstd.first.first;
                }
                IterationPlots.push_back(CreateAndFillWaveform(evt,event->Channels[ch],name.c_str(),title.c_str()));
                Iter++;
            } 
            ...
            if(ImEvent==true)
            {
                ...
                std::cout<<"EVENT   "<<testmeanstd.first.first<<"   "<<testmeanstd.first.second<<"   "<<testmeanstd.second.first<<"   "<<testmeanstd.second.second<<"  " <<testmeanstd.second.second/testmeanstd.first.second<<std::endl;
