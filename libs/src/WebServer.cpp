  ...
  m_server.setOnConnectionCallback([this](std::shared_ptr<ix::WebSocket> webSocket, std::shared_ptr<ix::ConnectionState> connectionState) {
    webSocket->setOnMessageCallback([this, webSocket, connectionState](const ix::WebSocketMessagePtr msg) {
      if(msg->type == ix::WebSocketMessageType::Open)
      {
        ...
        for(auto it: msg->openInfo.headers) { std::cerr << it.first << ": " << it.second << std::endl; }
      ...
      else if(msg->type == ix::WebSocketMessageType::Message)
      {
        ...
        if(ok)
        {
          if(value["Type"].asString() == "Command") { command = value["Command"].asString(); }
        ...
        else
        {
          ...
          val["Type"]    = "Status";
          val["Status"]  = "ALERT";
          val["Message"] = "<h1> Your command is not readeable !! Do you speak JSON !! "
                           "I don't understand Klingon yet !!! </h1>";
          std::stringstream s;
          writer->write(val, &s);
          webSocket->send(s.str());
        }
        /*std::cout<<":::::::"<<std::string(msg->str)<<":::::::"<<std::endl;
               ...
               std::string errs;
               std::ifstream
               message(std::string(msg->str),std::ifstream::binary); bool ok =
               Json::parseFromStream(builder,message,&obj,&errs); if ( !ok )
               {
                   std::cout  << errs << "\n";
               }
        ...
        /* Json::CharReaderBuilder builder;
             ...
             if (msg->str == "Where")
             {
                 std::cout<<"I TRIGGER" <<msg->str<<"   "<<where<<"  "<<command
             <<std::endl; webSocket->send(where.c_str());
                 }
             else
             {
