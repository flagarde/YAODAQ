...
int ParseConfigFile(FILE* f_ini, WaveDumpConfig_t& WDcfg)
{
  ...
  while(!feof(f_ini))
  {
    ...
    if(strstr(str, "OPEN") != NULL)
    {
      ...
      if(strcmp(str1, "USB") == 0) WDcfg.LinkType = CAEN_DGTZ_USB;
      ...
      if(WDcfg.LinkType == CAEN_DGTZ_USB) WDcfg.ConetNode = 0;
      else
    ...
    if(strstr(str, "DRS4_FREQUENCY") != NULL)
    {
      ...
      if(PrevDRS4Freq != WDcfg.DRS4Frequency) ret |= 0x1;
      continue;
    }
    ...
    if(strstr(str, "CORRECTION_LEVEL") != NULL)
    {
      ...
      read = fscanf(f_ini, "%s", str1);
      if(strcmp(str1, "AUTO") == 0) WDcfg.useCorrections = -1;
      else
      {
        int   gr = 0;
        char  Buf[1000];
    ...
    {
      read = fscanf(f_ini, "%s", str1);
      if(strcmp(str1, "YES") == 0) WDcfg.FastTriggerEnabled = 1;
      else if(strcmp(str1, "NO") != 0)
        printf("%s: invalid option\n", str);
      continue;
    }
    ...
    if(strstr(str, "PULSE_POLARITY") != NULL)
    {
      ...
      read                         = fscanf(f_ini, "%s", str1);
      if(strcmp(str1, "POSITIVE") == 0) pp = CAEN_DGTZ_PulsePolarityPositive;
      else if(strcmp(str1, "NEGATIVE") == 0)
        pp = CAEN_DGTZ_PulsePolarityNegative;
      else
        printf("%s: Invalid Parameter\n", str);

      for(i = 0; i < MAX_SET; i++)  /// polarity setting (old trigger edge)is
    ...
    if(strstr(str, "GROUP_TRG_ENABLE_MASK") != NULL)
    {
      ...
      if(ch == -1)
        for(i = 0; i < MAX_SET; i++) WDcfg.GroupTrgEnableMask[i] = val & 0xFF;
      else
    ...
    if(strstr(str, "CHANNEL_TRIGGER") != NULL)
    {
      ...
      if(ch == -1)
        for(i = 0; i < MAX_SET; i++) WDcfg.ChannelTriggerMode[i] = tm;
      else
