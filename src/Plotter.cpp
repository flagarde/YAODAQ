      ...
      if (dat.Event742->GrPresent[gr]) {
        for (std::size_t ch = 0; ch < 9; ch++) {
          ...
          if (Size <= 0)
            continue;
          for (int j = 0; j < Size; j++) {
            histos_histos[gr * 9 + ch].Fill(dat.Event742->DataGroup[gr].DataChannel[ch][j]);
          }
        }
      }
    }
      ...
      if (dat.Event742->GrPresent[gr]) {
        for (std::size_t ch = 0; ch < /*9*/3; ch++) {
          ...
          if (Size <= 0)
            continue;
          for (int j = 0; j < Size; j++) {
            histos[gr * 9 + ch].Fill(j, dat.Event742->DataGroup[gr].DataChannel[ch][j]*ti);
          }
        }
      }
    }
      ...
      if (dat.Event742->GrPresent[gr]) {
        for (std::size_t ch = 0; ch < 9; ch++) {
          ...
          for (int j = 0; j < Size; j++) {
              ...
              graphs[gr * 9 + ch].SetPoint(j,j*tickSize, dat.Event742->DataGroup[gr].DataChannel[ch][j]*ti);
            histos[gr * 9 + ch].Fill(j, dat.Event742->DataGroup[gr].DataChannel[ch][j]*ti);
          }
        }
      }
