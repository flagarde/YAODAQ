...
const char* CAENDPPException::errorStrings(const int_least32_t& code)
{
  switch(code)
  {
    ...
    case CAENDPP_RetCode_BadHistoState:
      return "The given histogram is an invalid state (ex. 'done' while it shouldn't)";
...
CAENDPPException::CAENDPPException(const int_least32_t& code, const source_location& location): Exception(code, errorStrings(code), location)
{
