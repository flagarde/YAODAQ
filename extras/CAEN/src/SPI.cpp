...
void SPI::select()
{
  CAENCommException(CAENComm_Write32(m_Handle, m_SPI_SELECT_REG_ADDR, 0x2));
}
...
void SPI::unselect()
{
  CAENCommException(CAENComm_Write32(m_Handle, m_SPI_SELECT_REG_ADDR, 0x3));
}

std::uint8_t SPI::read()
{
  std::uint32_t reg{0};
  CAENCommException(CAENComm_Read32(m_Handle, m_SPI_DATA_REG_ADDR, &reg));
  reg = (reg & 0xFF);
  return reg;
}
...
void SPI::write(const std::uint8_t& data)
{
  std::uint32_t reg{0};
  reg = data & 0xFF;
  CAENCommException(CAENComm_Write32(m_Handle, m_SPI_DATA_REG_ADDR, reg));
}
...
void SPI::write_block(std::vector<std::uint8_t>& buf)
{
  ...
  CAENCommException(CAENComm_MultiWrite32(m_Handle, &m_Addrs[0], buf.size(),
                                      &m_Datas[0],
...
std::vector<std::uint8_t> SPI::read_block(const std::uint32_t& len)
{
  ...
  CAENCommException(CAENComm_MultiRead32(m_Handle, &m_Addrs[0], len, &m_Datas[0],
                                     (CAENComm_ErrorCode*)&m_Errors[0]));
