...
public:
  WebsocketClient(
      const ix::WebSocketHttpHeaders& header = ix::WebSocketHttpHeaders());
  ~WebsocketClient();
  ...
  send(const std::string& data, bool binary = false,
...
private:
  ...
  std::function<void(const std::shared_ptr<ix::WebSocketMessage>)>
      m_SocketMessagePtr{nullptr};
};
