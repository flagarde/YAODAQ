...
WebsocketClient::WebsocketClient(const ix::WebSocketHttpHeaders &header)
    : m_Headers(header) {
  ix::initNetSystem();
}

void WebsocketClient::setExtraHeader(const std::string &key,
                                     const std::string &value) {
  m_Headers[key.c_str()] = (' ' + value).c_str();
}

void WebsocketClient::setOnMessageCallback(
    std::function<void(const std::shared_ptr<ix::WebSocketMessage>)> m_func) {
...
void WebsocketClient::stop() { m_WebSocket.stop(); }

ix::WebSocketSendInfo
WebsocketClient::send(const std::string &data, bool binary,
                      const ix::OnProgressCallback &onProgressCallback) {
  return m_WebSocket.send(data, binary, onProgressCallback);
}

ix::WebSocketSendInfo
WebsocketClient::sendBinary(const std::string &text,
                            const ix::OnProgressCallback &onProgressCallback) {
  return m_WebSocket.sendBinary(text, onProgressCallback);
}

ix::WebSocketSendInfo
WebsocketClient::sendText(const std::string &text,
                          const ix::OnProgressCallback &onProgressCallback) {
  return m_WebSocket.sendText(text, onProgressCallback);
}

void WebsocketClient::setURL(const std::string &url) { m_Url = url; }
