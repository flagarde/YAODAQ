...
std::vector<std::uint32_t> CAENCommBoard::MultiRead32(std::uint32_t& Address,const int& nCycles)
{
  ...
  ret.reserve(nCycles);
  CAENCommError(CAENComm_MultiRead32(m_Handle,&Address,nCycles,&ret[0],erros));
...
std::vector<std::uint16_t> CAENCommBoard::MultiRead16(std::uint32_t& Address,const int& nCycles)
{
  ...
  ret.reserve(nCycles);
  CAENCommError(CAENComm_MultiRead16(m_Handle,&Address,nCycles,&ret[0],erros));
  for(std::size_t i=0;i!=nCycles;++i)
  {
      CAENCommError(static_cast<int>(erros[i]));
  }
...
{
  CAENComm_ErrorCode erros[nCycles];
  CAENCommError(CAENComm_MultiWrite16(m_Handle,&Address,nCycles,&data[0],erros));
  for(std::size_t i=0;i!=nCycles;++i)
  {
      CAENCommError(static_cast<int>(erros[i]));
  }
...
{
  CAENComm_ErrorCode erros[nCycles];
  CAENCommError(CAENComm_MultiWrite32(m_Handle,&Address,nCycles,&data[0],erros));
  for(std::size_t i=0;i!=nCycles;++i)
  {
      CAENCommError(static_cast<int>(erros[i]));
  }
