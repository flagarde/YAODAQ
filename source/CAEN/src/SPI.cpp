...
void SPI::write_block(std::vector<std::uint8_t>& buf) 
{
  ...
  CAENCommError(CAENComm_MultiWrite32(m_Handle,&m_Addrs[0],buf.size(),&m_Datas[0],(CAENComm_ErrorCode *)&m_Errors[0]));
}
...
std::vector<std::uint8_t> SPI::read_block(const std::uint32_t& len) 
{
  ...
  CAENCommError(CAENComm_MultiRead32(m_Handle,&m_Addrs[0],len,&m_Datas[0],(CAENComm_ErrorCode *)&m_Errors[0]));
  for(std::size_t i = 0; i < len; ++i) 
  {
