...
std::string Pulser::returnIOSources(const unsigned char& source)
{
  ...
  else if(source==cvInputSrc0) return "InputSrc0";
  else if(source==cvInputSrc1) return "InputSrc1";
  else if(source==cvCoincidence) return "Coincidence";
  else if(source==cvVMESignals) return "VMESignals";
  else if(source==cvMiscSignals) return "MiscSignals";
  else throw -1;
}
...
std::string Pulser::returnTimeUnits(const unsigned char& source)
{
  ...
  else if(source==cvUnit1600ns) return "1600ns";
  else if(source==cvUnit410us) return "410us";
  else if(source==cvUnit104ms) return "104ms";
  else throw -1;
}
...
unsigned char Pulser::returnIOSources(const std::string& source)
{
  ...
  else if(source=="InputSrc0") return cvInputSrc0;
  else if(source=="InputSrc1") return cvInputSrc1;
  else if(source=="Coincidence") return cvCoincidence;
  else if(source=="VMESignals") return cvVMESignals;
  else if(source=="MiscSignals") return cvMiscSignals;
  else throw -1;
}
...
unsigned char Pulser::returnTimeUnits(const std::string& source)
{
  ...
  else if(source=="1600ns") return cvUnit1600ns;
  else if(source=="410us") return cvUnit410us;
  else if(source=="104ms") return cvUnit104ms;
  else throw -1;
}
...
{
  CAENVMEError(CAENVME_SetPulserConf(m_Handle,cvPulserA,p.getPeriod(),p.getWidth(),CVTimeUnits(p.returnTimeUnits()),p.getPulseNbr(),CVIOSources(p.returnIOSourcesStart()),CVIOSources(p.returnIOSourcesStop())));
}

void CAENVMEBoard::SetPulserBConf(Pulser& p)
{
  CAENVMEError(CAENVME_SetPulserConf(m_Handle,cvPulserB,p.getPeriod(),p.getWidth(),CVTimeUnits(p.returnTimeUnits()),p.getPulseNbr(),CVIOSources(p.returnIOSourcesStart()),CVIOSources(p.returnIOSourcesStop())));
}
...
Pulser CAENVMEBoard::GetPulserAConf()
{
  ...
  CVIOSources stop;
  Pulser p;
  CAENVMEError(CAENVME_GetPulserConf(m_Handle,cvPulserA,&period,&width,&units,&pulseno,&start,&stop));
  p.setPeriod(period);
  p.setWidth(width);
...
Pulser CAENVMEBoard::GetPulserBConf()
{
  ...
  CVIOSources stop;
  Pulser p;
  CAENVMEError(CAENVME_GetPulserConf(m_Handle,cvPulserB,&period,&width,&units,&pulseno,&start,&stop));
  p.setPeriod(period);
  p.setWidth(width);
  p.setPulseNbr(pulseno);
  p.setTimeUnits(units);
